...
- Updates section: a dated dev-update log generated by the `zensical-updates`
  ...
  pre-build step. Post sources live at `updates/<slug>.md` outside `docs/`; the
  generator writes the section into the gitignored `docs/updates/`. The build is
  now two-step: `zensical-updates build` then `zensical build --clean --strict`.
- A Week-0 dev-update scaffold at `updates/week-0.md` (front matter, headings,
